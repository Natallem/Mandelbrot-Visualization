...
sub_image::sub_image(const complex &vertex, size_t sub_image_degree, double scale) :
    ...
    precise_t step = precise_t(1 << sub_image_degree) / 2;
...
void sub_image::render_sub_image(std::atomic<uint64_t> &queue_version, uint64_t cur_version) {
    ...
    if (cur_version == queue_version && sub_image_degree != cur_degree) {
        size_t step = sub_image_size / (cur_size * 2);
...
size_t sub_image::color_value(complex point) {
    if (point.real() == 1. || point.real() == 0 || point.imag() == 0 || point.imag() == 1)
        return 255;
    complex z(0, 0);
...
void sub_image::update_QImage(QImage &img, size_t step, size_t size, std::atomic<uint64_t> &queue_version,
