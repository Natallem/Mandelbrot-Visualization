...
void render_system::worker_proc() {
    while (true) {
        ...
        int cur_size = -1;
        {
            std::lock_guard lock(queue.version_mutex);
...
QImage render_system::getImage(const complex &c, double scale, int step, int size) {
    int h = size;
    int w = size;
...
render_system::render_system(size_t number_of_workers, int sub_image_size, double scale) :
        queue(sub_image_size, scale) {
    for (int i = 0; i < number_of_workers; i++)
        workers.emplace_back([this]() { worker_proc(); });
}

void render_system::close() {
    queue.close();
